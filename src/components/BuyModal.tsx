...
export const BuyModal: FC<Props> = ({isOpen, onClose}) => {
  ...
  useEffect(()=>{
    if(!account) return;
    dispatch(getCoins(account.address));
  },[account])

  const [total, setTotal] = useState<number>(0);

  const handleCoinSelect = (selected: null | MultiValue<{ label: string; value: string; }>) => {
    setTotal(selected?.reduce((acc, option) => (acc+Number(option.value)), 0) || 0);
  };
      ...
      <ModalBody>
        <Stack gap={6}>
          <FormControl>
            ...
            <Select onChange={handleCoinSelect} isMulti options={
              coins.map(coin => ({
